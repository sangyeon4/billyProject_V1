	...
	<form name="frm" method="POST" action="billeyModAction"
		...
		<div class="mb-3">
			<label for="dateSelect" class="form-label">날짜 선택 </label><br> <input
				...
				placeholder="${bvo.getgStrDate()}" value="${bvo.getgStrDate()}">
			<input type="text" name="gEndDate" id="to" autocomplete="off"
		...
		<div class="mb-3">
			<label for="goodsFile" class="form-label">사진올리기</label> <br>
			<%
				ArrayList list = (ArrayList) request.getAttribute("attachList");
				for (int i = 0; i < list.size(); i++) {
					out.print("<button id='imgBtn" + i + "' type='button'>");
					out.print("<img src='download1?filename=" + list.get(i)
							+ "' style='width:150px; height:100px;' onclick=delImg" + i + "('" + list.get(i) + "')>");
					out.print("</button>");
					out.print("<br>");
				}
			%>
			<span id="img0"></span> <span id="img1"></span> <span id="delImg0"></span>
			<span id="delImg1"></span> <input type='hidden' class='form-control'
