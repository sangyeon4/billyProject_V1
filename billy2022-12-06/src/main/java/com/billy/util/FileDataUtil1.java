...
public class FileDataUtil1 {
   ...
   public FileSystemResource fileDownload(@RequestParam("filename") String fileName, HttpServletResponse response) {
      File file = new File(uploadPath1 + "/" + fileName);
      response.setContentType("application/download1; utf-8");
   ...
   public String[] fileUpload(MultipartFile[] file) throws IOException {
	   ...
	   for(int i = 0 ; i < file.length ; i++) {
		  if(file[i].getOriginalFilename()!="") {	//실제 file객체가 존재한다면
		      ...
		      File target = new File(uploadPath1, saveName);
